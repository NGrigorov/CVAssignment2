				...
				m_sphere_radius(1850)
{
	...
	createTrackbar("ERODE", VIDEO_WINDOW, &m_erode_power, 15);
	createTrackbar("DILATE", VIDEO_WINDOW, &m_dilate_power, 15);
	createFloorGrid();
	setTopView();
}
		...
		Camera* camera)
{
	...
	Mat manualsegmentation = imread(path + "manualsegmentationprocessed.png");

	vector<Mat> ManSegchannels;
	split(manualsegmentation, ManSegchannels);// Split the HSV-channels for further analysis
	Mat tmp, foreground, background;

	//
	// Manual Segmentation via photoshop image
	//
	absdiff(ManSegchannels[0], camera->getBgHsvChannels().at(0), tmp);
	threshold(tmp, foreground, m_h_threshold, 255, CV_THRESH_BINARY);

	absdiff(ManSegchannels[1], camera->getBgHsvChannels().at(1), tmp);
	threshold(tmp, background, m_s_threshold, 255, CV_THRESH_BINARY);

	bitwise_xor(foreground, background, foreground);

	absdiff(ManSegchannels[2], camera->getBgHsvChannels().at(2), tmp);
	threshold(tmp, background, m_v_threshold, 255, CV_THRESH_BINARY);
	
	bitwise_xor(foreground, background, foreground);

	erode(foreground, foreground, getStructuringElement(1,Size(m_erode_power, m_erode_power)));
	dilate(foreground, foreground, getStructuringElement(1, Size(m_dilate_power, m_dilate_power)));
