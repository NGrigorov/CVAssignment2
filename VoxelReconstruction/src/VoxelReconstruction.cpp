		...
		m_data_path(dp), m_cam_views_amount(cva)
{
	...
	for (int v = 0; v < m_cam_views_amount; ++v)
	{
		...
		/*
		 * Create background.png image by averaging all pixels from the avi file. \
		 * 
		 */

		//Full path and frames array
		string path = full_path.str() + "background.avi";
		VideoCapture avgFrames = VideoCapture(path);


		Mat first, next, avgImage;

		double alpha = 0;
		double beta = 0;
		double counter = 0;

		bool succes = avgFrames.read(next);
		first = next;
		
		//First image is created.
		addWeighted(next, alpha, first, beta, 0.0, avgImage);

		//We loop over the rest of the frames.
		while (succes)
		{
			alpha = 1.0 / (counter + 1);
			beta = 1.0 - alpha;
			counter++;
			addWeighted(next, alpha, avgImage, beta, 0.0, avgImage);
			succes = avgFrames.read(next);
		}

		//Write back to png file.
		imwrite(full_path.str() + "background.png", avgImage);
